...
export function xmlToGraph(xml: string): Graph {
  ...
  const nodes: GraphNode[] = nodeList.filter(Boolean).map((nodeData: any) => {
    ...
    const g: Graph = { nodes, edges: edges.map(e => ({ id: e.id, source: e.source, target: e.target, role: e.role, sourceHandle: e.sourceHandle, targetHandle: e.targetHandle })) as any } as Graph;
    return g;
  } catch (error) {
