...
export const useProjectStore = create<ProjectStore>((set, get) => ({
  ...
  loadProject: async () => {
    try {
      ...
      const response = await fetch('BACKEND_URL/api/files?graphs=true');
      ...
      if (response.ok) {
        ...
        await fetch('BACKEND_URL/api/backend/storage/initialize', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' }
        });
        
        // Also trigger graph API refresh to ensure it has the latest data
        await fetch('BACKEND_URL/api/backend/graph-api', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  loadGraph: async () => {
    try {
      ...
      const response = await fetch('BACKEND_URL/api/backend/graph-api', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'refresh' })
      });
