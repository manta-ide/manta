...
export class CodexProvider implements Provider {
  ...
  async run(opts: RunOptions): Promise<number> {
    ...
    try {
      ...
      // Select MCP toolset based on job kind (graph-builder, graph-editor vs read-only)
      envMap.MANTA_MCP_TOOLSET = jobKind === 'graph-builder' ? 'graph-builder' : (jobKind === 'graph-editor' ? 'graph-editor' : 'read-only');
    ...
    if (!env.MANTA_MCP_TOOLSET) env.MANTA_MCP_TOOLSET = (jobKind === 'graph-builder' ? 'graph-builder' : (jobKind === 'graph-editor' ? 'graph-editor' : 'read-only')) as any;
    try {
