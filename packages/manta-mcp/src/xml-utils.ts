...
function toPropTypeAttr(p: any): string {
  const t = p?.type;
  if (!t) return 'string';
...
function valueToText(p: any): string {
  const v = p?.value;
...
function generateFieldXml(field: any, fieldValue: any): string {
...
function generateNestedXml(p: any): string {
  ...
  if (type === 'object') {
    ...
    if ((!itemFields || itemFields.length === 0) && p?.item && typeof p.item === 'object' && !Array.isArray(p.item)) {
      const itemMap = p.item as Record<string, any>;
    ...
    const itemXml = effectiveItems.map((item: any, index: number) => {
      ...
      const itemFieldXml = fieldsToUse.map((field: any) => {
...
function optionsToText(p: any): string {
  const options = p?.options;
  if (!options || !Array.isArray(options)) return '';
  try { return JSON.stringify(options); } catch { return ''; }
}

export function graphToXml(graph: any): string {
    ...
    const props = Array.isArray(n.properties) && n.properties.length > 0
      ? `\n      <props>\n${n.properties.map((p: any) => {
          ...
          if (propType === 'object' || propType === 'object-list') {
            ...
            const optionsXml = options.map((option: any) => `          <option>${escapeXml(String(option))}</option>`).join('\n');
...
export function xmlToGraph(xml: string): any {
  try {
    ...
    const nodes = nodeList.filter(Boolean).map((nodeData: any) => {
